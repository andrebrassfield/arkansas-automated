...
for (const file of htmlFiles) {
  ...
  if (decodedTitle.length > 60) {
    titlesOver60++;
    // console.log("Title > 60: " + decodedTitle.length + " " + decodedTitle);
  }
  if (decodedDesc.length > 160) {
    descriptionsOver160++;
    // console.log("Desc > 160: " + decodedDesc.length + " " + decodedDesc);
  }
}

console.log(JSON.stringify({ titlesOver60, descriptionsOver160 }));
